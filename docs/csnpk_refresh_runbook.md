...
- 周一至周五目标为当前 ISO 周；北京时间周六 00:00 起目标切换到下一 ISO 周。周末巡检必须先创建并核验下一周 US/A股两页，再切换共享导航；缺页或门禁失败记 `weekly_roll_forward_blocked`，不能继续把已结束事件称为本周最新。
...
8. 涉及宏观雷达、财政风险或第37周美股事件页时，在页面生成后执行 `python3 scripts/render_near_term_macro.py`，从雷达 `data.json.near_term` 重建静态宏观补充；保留其独立核验时间，不能推进收盘模型日期。事件实际结果未回填时必须保留空值和待核验状态，不能宣称闭环已完成。
