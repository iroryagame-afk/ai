...
def compact(x):
 a=x['A_pass']
 return {'code':x['code'],'name':x['name'],'classes':x['source'],'industry':x['industry'],'stage':x['stage'],'change_pct':x['change_pct'],'volume_ratio':x['A']['current_ratio'] if a else x['volume_previous'],'volume_basis':'关K日' if a else '前一交易日','structure_date':x['A']['key_date'] if a else x['B']['fractal_date'],'pullback_days':[{'date':v['date'],'change_pct':v['change_pct'],'volume_vs_key':v['volume_vs_key']} for v in x.get('pullback_rows',[])],'m30_status':x['m30']['status'],'b_age_note':x.get('b_age_note','')}
def build(run,out):
 ...
 def tr(x,i):
  ...
  return '<tr data-top="'+str(x['code'] in top).lower()+'" data-search="'+esc(' '.join(vals)+' '+' '.join(x.get('other_groups',[])))+'">'+''.join('<td data-label="'+esc(h)+'">'+esc(v)+('<small class="group-note">备注：同时在 '+esc('、'.join(x['other_groups']))+'</small>' if h=='股票' and x.get('other_groups') else '')+'</td>' for h,v in zip(heads,vals))+'</tr>'
 heads=['排序','股票','类别／方向','结构节点','当前阶段','当日涨跌','量能比较','30分钟辅助','逐日量价／动能','跟踪']
